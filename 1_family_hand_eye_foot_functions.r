...
read.alspac <- function(mypath = 'C:\\Users\\Judith\\sciebo\\St Andrews\\projects\\gwas\\data_preparation\\data\\Paracchini_12Feb20.sav') {
...
# Purpose:  Format Alspac measures as Left-Right and
#           calculate lateralization indexes
#
# Inputs:   dataset: Alspac dataframe with lateralization
#           measures as Prefered-NonPreferedHand
#
...
#
  ...
  norm.data <- dataset %>% mutate(
    ...
    same.sex.hand = ifelse(test = gender == 2, yes = m.hand, no = p.hand),
    same.sex.foot = ifelse(test = gender == 2, yes = m.foot, no = p.foot),
    same.sex.eye = ifelse(test = gender == 2, yes = m.eye, no = p.eye),
      
    opp.sex.hand = ifelse(test = gender == 2, yes = p.hand, no = m.hand),
    opp.sex.foot = ifelse(test = gender == 2, yes = p.foot, no = m.foot),
    opp.sex.eye = ifelse(test = gender == 2, yes = p.eye, no = m.eye),
...
clean.alspac.eye.foot.hand <- function(mydata) {
  
  # Purpose:  Filter outliers and incomplete cases
  #
