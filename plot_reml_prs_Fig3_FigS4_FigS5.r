...
# read results
resid <- read.xlsx("results/2.reml/reml.hand.foot.eye.xlsx", sheetName = "figure3a")
resid <- resid %>%
...
left <- read.xlsx("results/2.reml/reml.hand.foot.eye.xlsx", sheetName = "figure3b")
left <- left %>%
  mutate(se.low = heritability - SE,
         se.up = heritability + SE,
         item = factor(item, levels = c("hand", "draw", "throw", "colour", "hold",  "cut", "hit", "foot", "kick", "pick", "stamp", "climb", "eye", "hole", "bottle")),
...
mixed <- read.xlsx("results/2.reml/reml.hand.foot.eye.xlsx", sheetName = "figure3c")
mixed <- mixed %>%
  mutate(se.low = heritability - SE,
         se.up = heritability + SE,
         item = factor(item, levels = c("hand", "draw", "throw", "colour", "hold",  "cut", "hit", "foot", "kick", "pick", "stamp", "climb", "eye", "hole", "bottle")),
...
Fig3 <- ggplot(data = resid, aes(x = item, fill = group, y = heritability)) +
  ...
  theme(panel.grid.major = element_blank(), 
        ...
        axis.text.x = element_text(angle = 45, size = 9, margin = margin(10,0,0,0), color = "dimgrey"),
...
tiff("outputs/plots/Fig3_final.tiff", units = "cm", width = 14, height = 6.5, res = 600)
Fig3
dev.off()

FigS4a <- ggplot(data = left, aes(x = item, fill = group, y = heritability)) +
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(ymin = pmax(0, se.low), ymax = se.up), width = 0.2, cex = 0.5) + 
  labs(y = "SNP heritability (+/- SE)",
...
FigS4b <- ggplot(data = mixed, aes(x = item, fill = group, y = heritability)) +
  geom_bar(stat = "identity") + 
  geom_errorbar(aes(ymin = pmax(0, se.low), ymax = se.up), width = 0.2, cex = 0.5) + 
  labs(y = "SNP heritability (+/- SE)",
       x = "item (mixed/ambidextrous vs right)",
       fill = "phenotype group") + 
  scale_fill_manual(values = c("#FEE090", "#A50026", "#74ADD1", "#313695")) + 
  ylim(0, 0.5) + 
  theme(panel.grid.major = element_blank(), 
...
tiff("outputs/plots/FigS4_final.tiff", units = "cm", width = 12, height = 13, res = 600)
ggarrange(FigS4a, FigS4b,
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
dev.off()
...
for (base in traits) {
  ...
  prs <- read.table(file = paste0("results/5.prs/", base, ".hand.foot.eye.resid.prsice"), header = TRUE)
...
#write.xlsx(supptable, file = "outputs/Table_S4.xlsx", sheetName = "cor", append = TRUE)

colnames(prs.r2) <- c(' ADHD',' ASD',' BIP',' SCZ',' EA',' IQ',
                      ' ADHD',' ASD',' BIP',' SCZ',' EA',' IQ')

tiff("outputs/plots/FigS5_final.tiff", units = "cm", width = 20, height = 18, res = 600)
par(mfrow = c(1,2))
